...
public class Patient {
    ...
    public Patient(String id, String processID, int ageInformation, String typeSurgery, List<Integer> numberPatientsPerSurgery) {
        ...
        double completionLikelihoodInfo;
        if (ageInformation <= 7 || ageInformation > 97) {
            completionLikelihoodInfo = 0;
        } else if (ageInformation > 7 && ageInformation <= 14) {
            completionLikelihoodInfo = (ageInformation - 7) * ((double) 1 / 7);
        } else if (ageInformation > 14 && ageInformation <= 90) {
            completionLikelihoodInfo = 1;
        } else if (ageInformation > 90 && ageInformation <= 97) {
            completionLikelihoodInfo = ((90 - ageInformation) * ((double) 1 / 7) + 1);
        } else {
            throw new IllegalArgumentException("Validation likelihood could not be calculated");
        }
        double observedFrequency;
        double completionLikelihoodTypeSurg = 1;
        switch (typeSurgery) {
            case "Gastroenterology":
                observedFrequency = ((double) (151.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(0));
                break;
            case "Ear/Nose/Throat":
                observedFrequency = ((double) (92.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(1));
                break;
            case "Urology/Endocrinology":
                observedFrequency = ((double) (87.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(2));
                break;
            case "Orthopedics":
                observedFrequency = ((double) (83.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(3));
                break;
            case "Women's clinic":
                observedFrequency = ((double) (81.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(4));
                break;
            case "Anesthesiology Procedures":
                observedFrequency = ((double) (75.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(5));
                break;
            case "Ophthalmology":
                observedFrequency = ((double) (69.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(6));
                break;
            case "Neurosurgery":
                observedFrequency = ((double) (57.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(7));
                break;
            case "Plastic/Hand":
                observedFrequency = ((double) (51.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(8));
                break;
            case "Cardio/Lung/Vascular":
                observedFrequency = ((double) (27.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(9));
                break;
            case "Common Procedures":
                observedFrequency = ((double) (6.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(10));
                break;
            case "Pediatrics":
                observedFrequency = ((double) (4.0 / 129));
                completionLikelihoodTypeSurg = 1 - ((double) observedFrequency / numberPatientsPerSurgery.get(11));
            default:
                break;
        }
        if (completionLikelihoodTypeSurg < 0) {
            completionLikelihoodTypeSurg = 0;
        }
        double completionLikelihood = completionLikelihoodInfo * completionLikelihoodTypeSurg;
        cancellationLikelihood = 1 - completionLikelihood;
